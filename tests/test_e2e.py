...
def _installed_profile(pb_mod, harness="claude", engine="ollama"):
    ...
    return {
        ...
        "ollama": {"models": [{"name": "qwen3-coder:30b", "id": "abc", "size": "19 GB", "modified": "x", "local": True}]} if engine == "ollama" else {"models": []},
        "lmstudio": {
            "present": engine == "lmstudio",
            "server_running": engine == "lmstudio",
            "server_port": 1234,
            "models": [{"path": "qwen/qwen3-coder-30b", "format": "mlx"}] if engine == "lmstudio" else [],
        },
        "llamacpp": {"present": False, "version": ""},
        "disk": {"path": str(pb_mod.STATE_DIR), "total_bytes": 1 << 40, "used_bytes": 0, "free_bytes": 1 << 40, "free_gib": 1024.0, "total_gib": 1024.0},
...
class TestPocBridgeCli:
    ...
    def test_ensure_config_codex_ollama_writes_state(self, isolated_state, monkeypatch, capsys):
        ...
        monkeypatch.setattr(pb, "smoke_test_ollama_model", lambda tag: {"ok": True, "response": "READY"})
        monkeypatch.setattr(pb, "configure_ollama_integration",
                            lambda target, model: {"target": target, "model": model, "state_dir": str(pb.STATE_DIR)})
        monkeypatch.setattr(sys, "argv", ["poc_bridge", "ensure-config", "codex", "--model", "qwen3-coder:30b"])
        pb.main()
        data = json.loads(capsys.readouterr().out)
    ...
    def test_adapters_subcommand_lists_both(self, isolated_state, monkeypatch, capsys):
        ...
        monkeypatch.setattr(pb, "command_version", lambda *a, **kw: {"present": True, "version": "x"})
        monkeypatch.setattr(pb, "parse_ollama_list", lambda: [])
        monkeypatch.setattr(pb, "lms_info", lambda: {"present": True, "server_running": True, "server_port": 1234, "models": []})
...
def _stub_subprocess_success(*args, **kwargs):
    return subprocess.CompletedProcess(args=args[0] if args else [], returncode=0, stdout="READY\n", stderr="")
...
class TestWizardFullFlow:
    ...
    def test_non_interactive_fails_cleanly_on_smoke_test_failure(self, isolated_state, monkeypatch):
        ...
        monkeypatch.setattr(pb, "machine_profile", lambda: _installed_profile(pb))
        monkeypatch.setattr(pb, "smoke_test_ollama_model",
                            lambda tag: {"ok": False, "error": "simulated failure"})
        rc = wiz.run_wizard(non_interactive=True)
        assert rc == 1
